...
namespace reg
{
    ...
    namespace assert {
        ...
        inline void success(LSTATUS error_code) {
            return success(static_cast<DWORD>(error_code));
        }

        inline void success(BOOL result) {
            if (!result)
                throw std::runtime_error(reg::ErrorCodeToString(GetLastError()));
        }
    ...
    [[nodiscard]]
    inline deleted_unique_ptr<HKEY> self_closing_registry_key() {
    ...
    [[nodiscard]]
    inline deleted_unique_ptr<HKEY> self_closing_registry_key(HKEY* handle) {
        return deleted_unique_ptr<HKEY>(handle, [](HKEY* h) {RegCloseKey(*h); });
    }

    [[nodiscard]]
    inline deleted_unique_ptr<HANDLE> self_closing_handle()
    {
        return deleted_unique_ptr<HANDLE>(new HANDLE, [](HANDLE* handle) {CloseHandle(*handle); });
    }

    [[nodiscard]]
    inline deleted_unique_ptr<TOKEN_USER> self_closing_token_user(HANDLE heap, DWORD flags, SIZE_T size)
    {
        return deleted_unique_ptr<TOKEN_USER>(
            static_cast<PTOKEN_USER>(HeapAlloc(GetProcessHeap(), flags, size)),
            [](PTOKEN_USER pToken) { HeapFree(GetProcessHeap(), 0, pToken); }
        );
    }

    [[nodiscard]]
    inline deleted_unique_ptr<SID> self_closing_sid(SID* pSID)
    {
        return deleted_unique_ptr<SID>(pSID, [](PSID sid) { FreeSid(sid); });
    }

    [[nodiscard]]
    inline deleted_unique_ptr<ACL> self_closing_acl(PACL pACL)
    {
        return deleted_unique_ptr<ACL>(pACL, [](PACL ptr) { LocalFree(ptr); });
    }

    [[nodiscard]]
    inline deleted_unique_ptr<ACL> self_closing_acl()
    {
        return deleted_unique_ptr<ACL>(new ACL, [](PACL ptr) { LocalFree(ptr); });
    }
    ...
    [[nodiscard]]
    inline deleted_unique_ptr<HKEY> open(HKEY machine, std::string_view key, REGSAM rights)
    {
        auto handle = self_closing_registry_key();
        DWORD result = NULL;
        ...
        result = RegOpenKeyEx(machine, key.data(), NULL, rights, handle.get());
    ...
    namespace security
    {
        /// <summary>Enables a given privilege for the current process.</summary>
        /// <param name="privilege">A string from the list of "NT Defined Privileges" defined in winnt.h</param>
        inline void enable_privilege(std::string_view privilege)
        {
            auto hToken = self_closing_handle();
            reg::assert::success(OpenProcessToken(GetCurrentProcess(), TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY, hToken.get()));

            LUID luid;
            reg::assert::success(LookupPrivilegeValue(NULL, privilege.data(), &luid));
            ...
            reg::assert::success(AdjustTokenPrivileges(*hToken, FALSE, &tp, sizeof(TOKEN_PRIVILEGES), NULL, NULL));
            if (GetLastError() == ERROR_NOT_ALL_ASSIGNED)
            {
                throw reg::except::unknown_error();
            }
        }

        inline deleted_unique_ptr<SID> init_sid()
        {
            PSID pAdminSID = NULL;
            SID_IDENTIFIER_AUTHORITY SIDAuthNT = SECURITY_NT_AUTHORITY;
            reg::assert::success(AllocateAndInitializeSid(&SIDAuthNT, 2, SECURITY_BUILTIN_DOMAIN_RID, DOMAIN_ALIAS_RID_ADMINS, 0, 0, 0, 0, 0, 0, &pAdminSID));
            return reg::self_closing_sid(static_cast<SID*>(pAdminSID));
        }

        inline void take_ownership(HKEY machine, std::string_view key)
        {
            _check_key(machine, key);

            reg::security::enable_privilege(SE_TAKE_OWNERSHIP_NAME);

            auto hKey = reg::open(machine, key, WRITE_OWNER);

            auto adminSID = init_sid();

            SECURITY_DESCRIPTOR sd;
            reg::assert::success(InitializeSecurityDescriptor(&sd, SECURITY_DESCRIPTOR_REVISION));
            reg::assert::success(SetSecurityDescriptorOwner(&sd, adminSID.get(), FALSE));
            reg::assert::success(RegSetKeySecurity(*hKey, OWNER_SECURITY_INFORMATION, &sd));
        }

        inline void set_full_access_rights(HKEY machine, std::string_view key)
        {
            _check_key(machine, key);
            auto hKey = reg::open(machine, key, READ_CONTROL | WRITE_DAC);

            PACL pOldDacl = NULL;
            PSECURITY_DESCRIPTOR pSID = NULL;
            reg::assert::success(GetSecurityInfo(*hKey, SE_REGISTRY_KEY, DACL_SECURITY_INFORMATION, NULL, NULL, &pOldDacl, NULL, &pSID));

            EXPLICIT_ACCESS eia;
            CHAR admin_name[] = "Administrators";
            ...
            PACL pNewDacl = NULL;
            reg::assert::success(SetEntriesInAcl(1, &eia, pOldDacl, &pNewDacl));
            auto newDACL = self_closing_acl(pNewDacl);

            reg::assert::success(SetSecurityInfo(*hKey, SE_REGISTRY_KEY, DACL_SECURITY_INFORMATION, NULL, NULL, newDACL.get(), NULL));
        }

        // inline bool check_access_rights(HKEY machine, std::string_view key)
        // {
        //     _check_key(machine, key);
        // 
        //     auto hKey = reg::open(machine, key, READ_CONTROL);
        // 
        //     PACL pDacl = NULL;
        //     PSECURITY_DESCRIPTOR pSD = NULL;
        //     reg::assert::success(GetSecurityInfo(*hKey, SE_REGISTRY_KEY, DACL_SECURITY_INFORMATION, NULL, NULL, &pDacl, NULL, &pSD));
        // 
        //     PSID pAdminSid = NULL;
        //     DWORD cbSid = 0;
        //     SID_NAME_USE eUse;
        //     LookupAccountName(NULL, "Administrators", pAdminSid, &cbSid, NULL, NULL, &eUse);
        // 
        //     pAdminSid = (PSID)malloc(cbSid);
        //     LookupAccountName(NULL, "Administrators", pAdminSid, &cbSid, NULL, NULL, &eUse);
        // 
        //     for (DWORD i = 0; i < pDacl->AceCount; i++) {
        //         ACCESS_ALLOWED_ACE* pAce;
        //         if (!GetAce(pDacl, i, reinterpret_cast<LPVOID*>(&pAce))) {
        //             continue;
        //         }
        // 
        //         if (EqualSid(pAdminSid, &(pAce->SidStart)) && pAce->Mask & KEY_ALL_ACCESS) {
        //             return true;
        //         }
        //     }
        //     return false;
    ...
    namespace query
    {
        namespace {
            ...
            {
                auto handle = open(machine, key, KEY_QUERY_VALUE);
        ...
        inline std::tuple<DWORD, DWORD, DWORD, DWORD> key_info(HKEY machine, std::string_view key)
        {
            ...
            auto handle = reg::open(machine, key, KEY_QUERY_VALUE);
        ...
        inline std::vector<std::string> keys(HKEY machine, std::string_view key)
        {
            ...
            auto handle = reg::open(machine, key, KEY_QUERY_VALUE | KEY_ENUMERATE_SUB_KEYS);
    ...
    namespace update
    {
        namespace {
            ...
            {
                auto handle = reg::open(machine, key, KEY_SET_VALUE);
    ...
    namespace create {
        namespace {
            ...
            std::tuple<deleted_unique_ptr<HKEY>, Disposition> _create_key(HKEY machine, std::string_view key)
            {
                deleted_unique_ptr<HKEY> handle = nullptr;
                ...
                {
                    handle = self_closing_registry_key();
                    DWORD result = NULL;
                    // RegCreateKeyEx(
                    ...
                    result = RegCreateKeyEx(
                        ...
                        KEY_READ | KEY_WRITE,
                        NULL,
                        handle.get(), &disposition);

                    reg::assert::success(result);
                }
                return { std::move(handle), static_cast<Disposition>(disposition) };
            }
            ...
            template<typename T>
            std::tuple<deleted_unique_ptr<HKEY>, Disposition> item(HKEY machine, std::string_view key, std::string_view value, T data) {
                if (key_exists(machine, key))
                {
                    auto handle = reg::open(machine, key, KEY_WRITE);

                    if (value_exists(machine, key, value))
                        return { std::move(handle), Disposition::EXISTS_VALUE };

                    else
                    {
                        reg::create::_create_value<T>(*handle, value, data);
                        return { std::move(handle), Disposition::CREATED_VALUE };
                    }
                }
                else
                {
        ...
        /// <param name='machine'>Root key in the hierarchy</param>
        /// <param name='key'>Subkey to the desired node</param>
        inline std::tuple<deleted_unique_ptr<HKEY>, Disposition> key(HKEY machine, std::string_view key) {
        ...
        inline std::tuple<deleted_unique_ptr<HKEY>, Disposition> number(HKEY machine, std::string_view key, std::string_view value, DWORD data = 0) {
        ...
        /// <param name='value'>Name of the value to be created</param>
        /// <param name='data'>Data to be assigned to value</param>
        inline std::tuple<deleted_unique_ptr<HKEY>, Disposition> string(HKEY machine, std::string_view key, std::string_view value, std::string_view data = "") {
    ...
    namespace remove
    {
        namespace
        {
            ...
            {
                auto handle = reg::open(machine, key, DELETE);
        ...
        inline bool values(HKEY machine, std::string_view key)
        {
            if (key_exists(machine, key))
            {
                auto handle = reg::open(machine, key, KEY_SET_VALUE | KEY_QUERY_VALUE);
